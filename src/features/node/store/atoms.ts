...
export const nodesAtom = atom<KnowledgeNode[]>([]);
export const edgesAtom = atom<KnowledgeEdge[]>([]);
export const selectedNodeIdAtom = atom<string | null>(null);

// Function to calculate expansion state based on children visibility
const calculateExpansionState = (nodeId: string, nodes: KnowledgeNode[]): ExpansionState => {
...
export const nodesWithDynamicExpansionAtom = atom((get) => {
  ...
  return nodes.map(node => ({
    ...
    data: {
      ...node.data,
      isExpanded: node.data.hasChildren ? calculateExpansionState(node.id, nodes) : ExpansionState.Collapsed,
    }
  }));
});
...
export const toggleNodeExpansionAtom = atom(
  ...
  (get, set, nodeId: string) => {
    ...
    if (currentExpansionState === ExpansionState.Collapsed || currentExpansionState === ExpansionState.Partial) {
      ...
      const hideNodeAndDescendants = (nodeId: string) => {
        const directChildren = updatedNodes.filter(n => n.data.parentId === nodeId);
        updatedNodes = updatedNodes.map(node => {
          if (node.id === nodeId || directChildren.some(child => child.id === node.id)) {
...
export const initializeNodesAndEdgesAtom = atom(
  null,
  (get, set, { nodes, edges }: { nodes: KnowledgeNode[], edges: KnowledgeEdge[] }) => {
    set(nodesAtom, nodes);
